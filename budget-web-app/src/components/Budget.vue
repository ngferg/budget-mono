...
const show_forward_button = computed(() => {
    ...
    return (
        year.value < current_year + 3 ||
        (year.value === current_year + 3 && month.value < 12)
    );
});
...
const new_line_item = async (category_id) => {
    ...
    if (
        isNaN(item_amounts.value[category_id]) ||
        item_amounts.value[category_id] <= 0
    ) {
...
const expense_breakdown = computed(() => {
    ...
    const totals = expense_cats
        .map((cat) => ({
            name: cat.name,
            total: (budget.value[cat.id] || []).reduce(
...
const donut_paths = computed(() => {
    ...
    return expense_breakdown.value.map(({ start, end, color }) => {
        ...
        const x1 = cx + ro * Math.cos(start),
            y1 = cy + ro * Math.sin(start);
        const x2 = cx + ro * Math.cos(end),
            y2 = cy + ro * Math.sin(end);
        const x3 = cx + ri * Math.cos(end),
            y3 = cy + ri * Math.sin(end);
        const x4 = cx + ri * Math.cos(start),
            y4 = cy + ri * Math.sin(start);
        return {
            color,
...
const income_total = computed(() => {
    ...
    return (budget.value[categories.value[0]?.id] || []).reduce(
        (sum, item) => sum + item.amount,
        0,
    );
});
...
const expense_total = computed(() => {
    ...
    return categories.value
        .slice(1)
        .flatMap((cat) => budget.value[cat.id] || [])
...
<template>
    ...
    <div v-if="budget !== null" class="card">
        <h2 class="justify-center">
            ...
            }}<button @click="next_month" v-if="show_forward_button">
                &gt;
            </button>
        </h2>
        ...
        <h3
            class="text-2xl font-bold text-emerald-300 mt-6 mb-4 border-b-2 border-emerald-500 pb-2"
        >
            Overview:
        </h3>
        <div v-if="expense_breakdown.length > 0" class="chart-container">
            ...
            <ul class="chart-legend">
                <li
                    v-for="cat in expense_breakdown"
                    :key="cat.name"
                    class="legend-item"
                >
                    <span
        ...
        <div
            v-if="income_total > 0 || expense_total > 0"
            class="ie-bar-container"
        >
            <div class="ie-bar-row">
                <span class="ie-bar-label">Income</span>
                <div class="ie-bar-track">
                    <div
                        class="ie-bar-fill ie-income"
                        :style="{ width: income_bar_pct + '%' }"
                    ></div>
                </div>
                <span class="ie-bar-amount">{{
                    formatCents(income_total)
                }}</span>
            </div>
            <div class="ie-bar-row">
                ...
                <div class="ie-bar-track">
                    <div
                        class="ie-bar-fill ie-expense"
                        :style="{ width: expense_bar_pct + '%' }"
                    ></div>
                </div>
                <span class="ie-bar-amount">{{
                    formatCents(expense_total)
                }}</span>
            </div>
            <div class="ie-bar-summary">
                <span v-if="income_total >= expense_total" class="ie-surplus"
                    >Surplus:
                    {{ formatCents(income_total - expense_total) }}</span
                >
                <span v-else class="ie-shortfall"
                    >Shortfall:
                    {{ formatCents(expense_total - income_total) }}</span
                >
            </div>
        </div>

        <h3
            class="text-2xl font-bold text-emerald-300 mt-8 mb-4 border-b-2 border-emerald-500 pb-2"
        >
            Categories:
        </h3>
        <div>
            <h4
                ...
                :key="category.id"
                class="text-xl font-bold text-emerald-100 mt-6 mb-3"
            >
                {{ category.name }}:
                {{
                    formatCents(
                ...
                <ul>
                    <li v-for="item in budget[category.id]" :key="item.id">
                        ...
                        <button @click="delete_line_item(item.id)">-</button
                        ><button
                            @click="
                                open_edit_line_item_modal(
                                    item.id,
                                    item.description,
                                    item.amount,
                                )
                    ...
                    <li>
                        <input
                            ...
                            v-model="item_descriptions[category.id]"
                            :ref="
                                (el) => {
                                    if (el) descInputs[category.id] = el;
                                }
                            "
                        />
                        ...
                        <div class="amount-add-row">
                            <input
                                type="number"
                                placeholder="Amount"
                                v-model.number="item_amounts[category.id]"
                                @keydown.enter="new_line_item(category.id)"
                            />
                            <button @click="new_line_item(category.id)">
                                +
                            </button>
                        </div>
                    </li>
    ...
    <Teleport to="body">
        ...
        <div v-if="show_edit_modal" class="modal-container">
            <div class="modal-header">
                ...
                <button @click="close_edit_modal" class="close-button">
                    &times;
                </button>
            </div>
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input
                        type="number"
                        v-model.number="edit_item_amount"
                        step="0.01"
                        @keydown.enter="save_edit_line_item"
                    />
                </div>
